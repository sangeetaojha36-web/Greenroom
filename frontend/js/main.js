...
function resolveApiBaseUrl() {
  ...
  // If hosted on Vercel, Cloud Run, or any remote cloud domain, always use relative same-origin URLs
  if (hostname && hostname !== "localhost" && hostname !== "127.0.0.1" && !hostname.startsWith("192.168.")) {
    return "";
  }
  // If running locally via VS Code Live Server (port 5500), Vite (5173), or file://, route to local Node backend on 3000
  if (protocol === "file:" || (port && port !== "3000" && port !== "5000")) {
    const host = hostname || "localhost";
    return `http://${host}:3000`;
  }
  return ""; // same-origin when served by Express (port 3000) or Flask (port 5000)
}
...
async function loadCompanies(){
  try{
    ...
    const data = await res.json();
    if (!Array.isArray(data) || !data.length) throw new Error("Empty company bank");
    state.companies = data;
  }catch(e){
    console.warn("Backend API unreachable, using built-in company bank:", e.message);
    if (typeof FALLBACK_COMPANIES !== "undefined" && Array.isArray(FALLBACK_COMPANIES) && FALLBACK_COMPANIES.length > 0) {
      state.companies = FALLBACK_COMPANIES;
    }
    toast("Operating with built-in company question banks.");
  } finally {
    loadCustomBanks();
    renderCompanyGrid();
...
function getOfflineQuestionsForCompany(companyId, role, level = "mid", count = 6) {
  const bank = (typeof FALLBACK_QUESTIONS_MAP !== "undefined" && FALLBACK_QUESTIONS_MAP[companyId])
    ? FALLBACK_QUESTIONS_MAP[companyId]
    : null;
  const list = [];
  if (bank) {
    const roleQs = bank.roles[role] || Object.values(bank.roles)[0] || [];
    list.push(...roleQs);
    if (bank.behavioral) list.push(...bank.behavioral);
    if (bank.hr) list.push(...bank.hr);
  }
  if (list.length === 0) {
    list.push(
      { id: "fallback_1", q: `Walk me through a challenging software project you engineered. What technical trade-offs did you make?`, type: "technical", level: "mid", keywords: ["architecture", "scale", "trade-offs", "database"] },
      { id: "fallback_2", q: `How do you diagnose and resolve latency bottlenecks in a high-throughput microservice?`, type: "technical", level: "mid", keywords: ["profiling", "latency", "cache", "indexing"] },
      { id: "fallback_3", q: `Tell me about a time you had a technical disagreement with a team member. How was it resolved?`, type: "behavioral", level: "mid", keywords: ["situation", "task", "action", "result"] }
    );
  }
  const matched = list.filter(q => q.level === level);
  const pool = matched.length >= count ? matched : list;
...
async function loadCallSheet(companyId){
  ...
  try{
    ...
    console.warn("Using offline call sheet questions:", e.message);
    const bank = (typeof FALLBACK_QUESTIONS_MAP !== "undefined" && FALLBACK_QUESTIONS_MAP[companyId]) ? FALLBACK_QUESTIONS_MAP[companyId] : null;
    if (bank) {
      const roleQs = bank.roles[state.selectedRole] || Object.values(bank.roles)[0] || [];
      const groups = [
        ["Technical", roleQs],
  ...
  try{
    if (company.isCustom) {
      ...
      try {
        ...
        console.warn("Questions fetch failed from backend, using built-in questions:", fetchErr.message);
        state.questions = getOfflineQuestionsForCompany(company.id, state.selectedRole, lvl, state.qCount);
      }
    }
...
async function submitAnswer(){
  ...
  try{
    ...
    console.warn("Backend evaluation unreachable, computing client-side assessment:", e);
    const words = answer.trim().split(/\s+/).filter(Boolean);
    const wordCount = words.length;
    const kw = q.keywords || [];
    const lowerAns = answer.toLowerCase();
    const matchedKw = kw.filter(k => lowerAns.includes(k.toLowerCase()));
    const kwRatio = kw.length > 0 ? (matchedKw.length / kw.length) : 0.65;
    const relevanceScore = Math.min(95, Math.round(45 + (kwRatio * 40) + Math.min(10, wordCount / 8)));
    const structureScore = q.type === "behavioral" 
      ? (lowerAns.includes("situation") || lowerAns.includes("action") || lowerAns.includes("result") ? 84 : 70)
      : (wordCount > 40 ? 82 : 65);
    const fluencyScore = Math.max(55, Math.min(95, 88 - (lowerAns.match(/\b(um|uh|like|you know)\b/g) || []).length * 4));
    const confidenceScore = Math.min(94, Math.max(62, 75 + (wordCount > 45 ? 10 : 0)));
    const overall = Math.round((relevanceScore * 0.4) + (structureScore * 0.25) + (fluencyScore * 0.2) + (confidenceScore * 0.15));
    ...
    const result = {
      ...
      verdict: overall >= 78 ? "Strong Hire / Pass" : overall >= 65 ? "Hire / Meets Standard" : "Lean No Hire / Needs Work",
      feedback: [
        `Key concepts discussed: ${matchedKw.join(", ") || "Foundational topic coverage"}.`,
        wordCount >= 40 ? "Articulated with good technical vocabulary and depth." : "Consider expanding your answer with more concrete architectural examples.",
        "Scored via offline fallback engine. In VS Code, run 'npm start' in terminal for full Gemini/NLP grading."
      ],
      question_text: q.q,
    ...
    revealScorecard(result);
    checkAchievements();
    toast("Answer scored via offline engine! In VS Code, run 'npm start' for full cloud sync.");
  }finally{
...
function initEventListeners(){
  ...
  // Candidate Profile & Database Records Listeners
  $("#navUserChip")?.addEventListener("click", () => {
    openDbRecordsModal();
  });
  $("#dbSaveProfileBtn")?.addEventListener("click", () => {
...
function updateCandidateNavChip() {
  const name = state.candidateName || localStorage.getItem("gr_user_name") || "Candidate";
  const role = state.candidateRole || localStorage.getItem("gr_user_role") || "Software Engineer";
  ...
  if (navUserAvatar) navUserAvatar.textContent = (name.charAt(0) || "C").toUpperCase();
  if (dbName && !dbName.value) dbName.value = name;
  if (dbRole && !dbRole.value) dbRole.value = role;
}
